...
class CompleteWeiboScraper:
    ...
    def generate_complete_markdown(self, weibos):
        ...
        filename = f"姜汝祥_完整微博内容_含图片链接_{timestamp}.md"
                    ...
                    if 'images' in weibo and weibo['images']:
                        ...
                        for idx, img in enumerate(weibo['images'], 1):
                            ...
                            if img.get('local_file'):
                                f.write(f"   **本地文件**: `{self.images_dir}/{img['local_file']}`\n")
                                # 如果是Markdown支持的图片格式，直接嵌入
                                if img['local_file'].lower().endswith(('.jpg', '.jpeg', '.png', '.gif')):
                                    f.write(f"   ![图片{idx}]({self.images_dir}/{img['local_file']})\n")
    ...
    def run(self):
        ...
        if weibos:
            print("\n📝 生成完整Markdown文档...")
            filename = self.generate_complete_markdown(weibos)
            
            # 统计
            total_images = sum(len(w.get('images', [])) for w in weibos)
            
            print(f"\n✅ 任务完成!")
            print(f"📄 文档文件: {filename}")
            print(f"📊 微博数量: {len(weibos)} 条")
            print(f"🖼️ 图片下载: {total_images} 张")
